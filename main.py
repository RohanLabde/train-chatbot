...
try:
    for train_no, route in TRAIN_DATA.items():
        if isinstance(route, list):
            ...
            logging.warning(f"🚨 Unexpected format for train {train_no}: Expected a list, got {type(route)}")
except Exception as e:
...
def chatbot():
    try:
        ...
        logging.error("❌ Invalid input format.", exc_info=True)
    ...
    if intent == "train_search" and source and destination:
        try:
            for train_no, stops in TRAIN_DATA.items():
                if isinstance(stops, list):
                    stations = [s.get("station_code") for s in stops if s.get("station_code")]
                    if source in stations and destination in stations:
                        ...
                        if src_index < dest_index:
                            trains_found.append({
                                "train_no": train_no,
                                "train_name": stops[0].get("train_name", "Unknown"),
                                "source": source,
